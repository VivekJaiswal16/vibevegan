export default function Hero() {
  return (
    <section style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'flex-start', padding: 'clamp(100px, 15vw, 140px) 5% 80px', position: 'relative', overflow: 'hidden' }}>
      ...
      <h1 style={{ fontFamily: "'Syne',sans-serif", fontSize: 'clamp(2.8rem,8vw,7.5rem)', fontWeight: 800, lineHeight: .95, letterSpacing: -3, maxWidth: 900, animation: 'fadeUp .8s .1s ease both' }}>
        Eat<br />
        <span style={{ color: 'var(--green)' }}>Different.</span><br />
        Live <span style={{ color: 'var(--lime)' }}>Loud.</span>
      </h1>

      <p style={{ marginTop: 28, fontSize: 'clamp(.95rem,2.5vw,1.15rem)', color: 'var(--sub)', maxWidth: 520, lineHeight: 1.6, fontWeight: 300, animation: 'fadeUp .8s .2s ease both' }}>
        Your choices are the most powerful thing you own. One meal at a time, one day at a time — let's change the world together.
      </p>

      <div style={{ marginTop: 40, display: 'flex', gap: 12, flexWrap: 'wrap', animation: 'fadeUp .8s .3s ease both' }}>
