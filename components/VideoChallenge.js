...
export default function VideoChallenge() {
  ...
  return (
    <section id="challenge" style={{ padding: '80px 5%', background: 'var(--dark)' }}>
      ...
      <div className="reveal" style={{ marginTop: 40, display: 'grid', gridTemplateColumns: mobile ? '1fr' : '1fr 1fr', gap: mobile ? 12 : 2 }}>
        <div style={{ background: 'var(--card)', padding: mobile ? '32px 24px' : '52px 44px', borderRadius: mobile ? 20 : '20px 0 0 20px' }}>
        ...
        <div style={{ background: 'var(--green)', padding: mobile ? '40px 24px' : '52px 44px', borderRadius: mobile ? 20 : '0 20px 20px 0', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center' }}>
          ...
          <a href="https://3minutes.wtf" target="_blank" rel="noreferrer"
            style={{ background: 'var(--dark)', color: 'var(--green)', padding: '13px 26px', borderRadius: 100, fontWeight: 700, fontSize: '.9rem', textDecoration: 'none', display: 'inline-block' }}>
            ▶ Take the Challenge
          </a>
