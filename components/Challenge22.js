...
export default function Challenge22() {
  return (
    <section id="22days" style={{ padding: '100px 5%', background: 'var(--muted)' }}>
      ...
      <div className="reveal" style={{ marginTop: 60, background: 'var(--card)', borderRadius: 24, overflow: 'hidden', display: 'grid', gridTemplateColumns: '1fr 1.2fr' }}>
        {/* Left — big number */}
        <div style={{ padding: '60px 50px', background: 'linear-gradient(135deg,#1a2b1a 0%,#0f1a0f 100%)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ fontFamily: "'Syne',sans-serif", fontSize: '5.5rem', fontWeight: 800, color: 'var(--green)', lineHeight: 1, letterSpacing: -3 }}>22</div>
        ...
        <div style={{ padding: '60px 50px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          {[
            ...
            <div key={i}>
              ...
              <p style={{ color: 'var(--sub)', fontSize: '.88rem', lineHeight: 1.5, marginBottom: i < 2 ? 0 : undefined }}>{w.desc}</p>
              {i < 2 && <div style={{ height: 1, background: 'rgba(255,255,255,.05)', margin: '28px 0' }} />}
            </div>
          ))}
        </div>
      </div>
