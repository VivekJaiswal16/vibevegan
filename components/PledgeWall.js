...
export default function PledgeWall({ initialPledges = [] }) {
  // Merge server-fetched pledges with defaults (defaults shown if DB is empty)
  ...
  return (
    <section id="pledge" style={{ padding: '100px 5%', background: 'var(--dark)' }}>
      ...
      <div className="reveal">
        ...
        <div style={{ marginTop: 48, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <input value={name} onChange={e => setName(e.target.value)} maxLength={60} placeholder="Your name"
            style={{ background: 'var(--card)', border: '1.5px solid rgba(255,255,255,.08)', borderRadius: 12, padding: '14px 18px', color: 'var(--text)', fontFamily: "'DM Sans',sans-serif", fontSize: '.92rem', outline: 'none', width: 200 }}
          ...
          <input value={reason} onChange={e => setReason(e.target.value)} maxLength={200} placeholder="I'm going vegan because..."
            style={{ background: 'var(--card)', border: '1.5px solid rgba(255,255,255,.08)', borderRadius: 12, padding: '14px 18px', color: 'var(--text)', fontFamily: "'DM Sans',sans-serif", fontSize: '.92rem', outline: 'none', flex: 1, minWidth: 220 }}
            onFocus={e => e.target.style.borderColor = 'var(--green)'}
            onBlur={e  => e.target.style.borderColor = 'rgba(255,255,255,.08)'}
            onKeyDown={e => e.key === 'Enter' && addPledge()}
          />
          <button onClick={addPledge} disabled={loading} style={{ background: 'var(--green)', color: 'var(--dark)', border: 'none', borderRadius: 12, padding: '14px 24px', fontWeight: 700, fontSize: '.92rem', cursor: loading ? 'not-allowed' : 'pointer', whiteSpace: 'nowrap', opacity: loading ? .7 : 1, fontFamily: "'DM Sans',sans-serif", transition: 'transform .2s' }}>
        ...
        {/* Pledge masonry wall */}
        <div style={{ marginTop: 36, columns: 3, gap: 16 }}>
          {pledges.map((p, i) => (
            <div key={p.id || i} style={{ breakInside: 'avoid', background: 'var(--card)', borderRadius: 14, padding: '20px 22px', marginBottom: 16, border: '1px solid rgba(255,255,255,.04)', animation: 'popIn .4s ease both' }}>
        ...
        <p style={{ fontFamily: "'Syne',sans-serif", fontSize: '1rem', color: 'var(--sub)', marginTop: 28 }}>
          <span style={{ color: 'var(--green)', fontWeight: 700 }}>{pledges.length}</span> people have pledged so far 💚
        </p>
      </div>
    </section>
  )
